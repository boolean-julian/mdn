...
def g(x):
	...
	r[5] = -.5*(x[0]-8)**2-2*(x[1]-4)**2-3*x[4]**2+x[5]+30
...
def func(x):
	return 0.5*np.absolute(np.dot(x,x))
...
def newtons_naive(curr):
	...
	complete = False
	while not complete:
		r = xs[i] - np.matmul(np.linalg.inv(hessf(xs[i])), gradf(xs[i]))
		xs.append(r)
		if np.linalg.norm(gradf(xs[i])) < 1:
			complete = True
		else:
			i += 1
		if i > 10000:
